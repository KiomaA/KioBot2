...
export default class TwitchRedemption extends Component {
   ...
   redeem(chatClient, channel, platform, name, lang, msg, remark) {
      ...
      if (item.vipOnly && remark.chatMessage.userInfo.isVip) {
         ...
         message =
            "本獎勵只限VIP兌換，請聯絡台主退還點數。 This reward is for VIP only. Please contact streamer to return your points.";
      }
      ...
      if (item.subOnly && remark.chatMessage.userInfo.isSubscriber) {
         ...
         message =
            "本獎勵只限訂閲者兌換，請聯絡台主退還點數。 This reward is for subscribers only. Please contact streamer to return your points.";
      }
      ...
      chatClient.say(
         channel,
         `感謝 ${name} 兌換${rewardNameZh}！ Thanks ${name} for redeeming ${rewardNameEn}!`,
      );
         ...
         if (!isNaN(duration)) {
            setTimeout(
               () => {
                  chatClient.say(
                     channel,
                     `兌換${rewardNameZh}時間已到！ Time's up for reward ${rewardNameEn}!`,
                  );
   ...
   handleAdminMessage(client, message, messageHandler) {
      ...
      switch (command) {
         case "update":
            this.updateRewardList();
            reply = "Reward list updated";
            break;
         case "mute":
            this.playSound = false;
            reply = "Reward sound muted";
            break;
         case "unmute":
            this.playSound = true;
            reply = "Reward sound enabled";
            break;
         default:
            reply = "Usage: update mute unmute";
            break;
      }
