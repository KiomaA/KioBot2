...
const testSoundFile = config.autoreply.testSoundFile;
...
export default class AutoReply extends Component {
   ...
   async updateReplyList() {
      ...
      const replies = rows.map((row) => {
         ...
         return {
            ...
            type: rawData[1],
            reply: rawData[2],
            ignoreAdmin: !!rawData[3],
            once: !!rawData[4],
            exceptions: exceptions,
            sound: rawData[6],
            soundCooldown: cooldown,
            replyLanguages: replyLanguages,
            skipLanguages: skipLanguages,
            disabled: !!rawData[10],
            subscriberReply: rawData[11],
            subscriberUniqueSound: !!rawData[12],
         };
      });
   ...
   replyMessage(client, channel, message, language, name, user, platform, isAdmin, remark) {
      for (const reply of this.replies) {
         ...
         if (message.toLowerCase().includes(reply.message)) {
            ...
            if (reply.replyLanguages.length >= 1 && !reply.replyLanguages.includes(language))
               continue;
               ...
               if (reply.soundCooldown) {
                  if (!this.soundTime[reply.type]) {
                     ...
                     if (nowSeconds - this.soundTime[reply.type] < reply.soundCooldown) {
                        playsound = false;
                        console.log(
                           `play sound in cooldown: ${reply.soundCooldown - (nowSeconds - this.soundTime[reply.type])} s remaining`,
                        );
                     } else {
   ...
   test() {
      if (testSoundFile && fs.existsSync("./" + testSoundFile)) {
         this.io.emit("autoreply", { file: testSoundFile });
         return "Auto-reply test sound played";
      }
      return "Auto-reply test sound file not found, please change environment variables.";
   }
   ...
   handleAdminMessage(client, message, messageHandler) {
      ...
      switch (command) {
         ...
         case "update":
            this.updateReplyList();
            reply = "Auto-reply updated";
            break;
         case "mute":
            this.mute = true;
            reply = "Auto-reply muted";
            break;
         case "unmute":
            this.mute = false;
            reply = "Auto-reply unmuted";
            break;
         case "resetUsers":
            reply = this.reset(true, false, false);
            break;
         case "resetCount":
            reply = this.reset(false, true, false);
            break;
         case "resetTime":
            reply = this.reset(false, false, true);
            break;
         case "reset":
            reply = this.reset(true, true, true);
            break;
         case "test":
            reply = this.test();
            break;
         default:
            reply = "Usage: update mute unmute resetUsers resetTime reset";
            break;
      }
