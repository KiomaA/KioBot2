...
export default class YoutubeLiveChat extends Component {
   // Deprecated, use YoutubeiLiveChat instead. This is kept for reference and backup.
   fetchTime = new Date();
   connected = false;
   initiated = false;
   ...
   handleAdminMessage(client, message, messageHandler) {
      ...
      switch (command) {
         case "connect":
            this.connect();
            reply = "(Note: deprecated, use !yi instead)";
            break;
         case "disconnect":
            this.disconnect();
            reply = "(Note: deprecated, use !yi instead)";
            break;
         default:
            reply = "(Note: deprecated, use !yi instead) Usage: connect disconnect";
            break;
      }
      if (reply) {
         console.log(reply);
